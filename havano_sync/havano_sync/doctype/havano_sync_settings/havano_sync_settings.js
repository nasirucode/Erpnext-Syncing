...
frappe.ui.form.on("Havano Sync Settings", {
	refresh(frm) {
		...
		// Fetch Item Prices and Exchange Rates button
		frm.add_custom_button(__("Fetch Item Prices and Exchange Rates"), function() {
			frappe.call({
				method: "havano_sync.havano_sync.api.sync.trigger_fetch_item_prices_and_exchange_rates",
				freeze: true,
				freeze_message: __("Fetching item prices and exchange rates..."),
				callback: function(r) {
					if (r.message) {
						frappe.show_alert({
							message: r.message.message || __("Item prices and exchange rates fetched successfully."),
							indicator: "green"
						}, 5);
					}
				},
				error: function(r) {
					frappe.show_alert({
						message: r.message.message || __("Failed to fetch item prices and exchange rates."),
						indicator: "red"
					}, 5);
				}
			});
