...
frappe.listview_settings['Sales Invoice'] = {
	...
	onload: function(listview) {
		...
		const resyncHandler = function() {
			frappe.confirm(
				__('Are you sure you want to resync all pending Sales Invoices? This will queue all pending documents for sync.'),
				function() {
					// Yes
					frappe.show_alert({
						message: __('Resyncing all pending Sales Invoices...'),
						indicator: 'blue'
					}, 5);
					...
					frappe.call({
						...
						callback: function(r) {
							if (r.message) {
								if (r.message.status === 'completed' || r.message.status === 'success') {
									...
									let message = __('Sales Invoices sync completed.');
									if (total_synced > 0) {
									...
									frappe.show_alert({
										message: __(r.message.message || 'Failed to resync Sales Invoices.'),
										indicator: 'red'
									}, 5);
								}
							} else {
								frappe.show_alert({
									message: __('Failed to resync Sales Invoices.'),
									indicator: 'red'
								}, 5);
							}
						},
						error: function(r) {
							frappe.show_alert({
								message: __('Error resyncing Sales Invoices. Please check error logs.'),
								indicator: 'red'
							}, 5);
						}
					});
