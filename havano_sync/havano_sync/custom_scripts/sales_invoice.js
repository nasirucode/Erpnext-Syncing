	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.name) {
			frm.add_custom_button(__('Resync Pending'), function() {
				frappe.show_alert({
					message: __('Resyncing Sales Invoice...'),
					indicator: 'blue'
				}, 5);
				frappe.call({
					...
					// freeze: true,
					// freeze_message: __('Resyncing Sales Invoice...'),
					callback: function(r) {
						if (r.message) {
							if (r.message.status === 'success') {
								frappe.show_alert({
									message: __(r.message.message || 'Sales Invoice has been queued for resync.'),
									indicator: 'green'
								}, 5);
								...
								frappe.show_alert({
									message: __(r.message.message || 'Failed to resync Sales Invoice.'),
									indicator: 'red'
								}, 5);
							}
						} else {
							frappe.show_alert({
								message: __('Failed to resync Sales Invoice.'),
								indicator: 'red'
							}, 5);
						}
					},
					error: function(r) {
						frappe.show_alert({
							message: __('Error resyncing Sales Invoice. Please check error logs.'),
							indicator: 'red'
						}, 5);
					}
				});
